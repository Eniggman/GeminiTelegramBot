...
| 🌐 Перевод            | `gemini-3.1-flash-lite-preview` |
...
|:----------------------|:------------------------------|:----------------:|:-------------------------------|
| `п` / `про` / `pro`   | 💎 Переключить на Gemini Pro  |   `2482`         | `п`                            |
| `ф` / `флеш` / `flash`| ⚡ Переключить на Gemini Flash |   `2489`         | `ф`                            |
| `к` / `картинка`      | 🎨 Генерация изображения      | `2508` / `2552`  | `к красивый закат`             |
| `к про`               | 💎 Модель картинок → Pro      |   `2519`         | `к про`                        |
| `к флеш`              | ⚡ Модель картинок → Flash     |   `2535`         | `к флеш`                       |
| `пр` / `перевод`      | 🌐 Перевод текста/фото        | `2342` / `2351`  | `пр hello world`               |
| `р` / `редактировать` | ✏️ Режим редактирования фото  |   `2562`         | `р`                            |
| `ю` / `ютуб` / `самари`  | 📺 YouTube саммари             | `2383` / `2393`  | `ю https://youtu.be/...`       |
| `превью` / `пре`      | 🖼️ YouTube превью             | `2436` / `2446`  | `превью https://youtu.be/...`  |
| `.`                   | 🧹 Сброс контекста            |   `2496`         | `.`                            |
| `выход` / `exit`      | ⏹️ Выход из режима            |   `2309`         | `выход`                        |
---
...
| `1628-2137` | **Фото и Документы** | `handle_photo()`, `process_album_delayed()`, `handle_document` |
| `2138-2878` | **Обработчик сообщений** | Диспетчер `handle_message()` и хелперы (Р, Пр, Ю, К) |
| `2879-3227` | **Инлайн-режим** | `handle_inline_query()`, `handle_chosen_inline_result()` |
| `3228-3514` | **Callback-и и Запуск** | `button_callback()`, `post_init()`, `main()` |
...
|:--------------------------|:------:|:-----------------------------|
| `delete_safe()`          |  290   | Безопасное удаление сообщений |
| `edit_image()`           |  946   | Редактирование фото/альбомов |
| `handle_photo()`         |  1632  | Обработка фото и альбомов    |
| `process_album_delayed()`|  1911  | Сборка и обработка альбомов  |
| `button_callback()`      |  3230  | Обработка кнопок под фото    |
